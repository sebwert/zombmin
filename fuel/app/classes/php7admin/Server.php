...
class Server 
{
    ...
    {
        $ip = Config::get()->getTelnetIP();
        $port = Config::get()->getTelnetPort();
        $this->telnet = new Telnet($ip, $port, 5, 'to end session.');
    ...
    public function kick($players, $reason = '')
    {
        ...
        foreach($players as $single_player) {
            $this->say(sprintf($say_template, $single_player, $reason));
    ...
    public function ban($players, $time = '1 day', $reason = '')
    {
        if(!is_array($players)) {
            $players = array($players);
        }
